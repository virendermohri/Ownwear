    ...
    <div className='container px-2 py-3 md:px-10 mx-auto '>
      ...
      <div className="px-2 w-full">
        <div className=" mb-4">
          ...
          <textarea id="address" name="address" cols='30' row='10' className="w-full bg-white rounded border border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out" />
        </div>
      </div>
      ...
      <div className="mx-aouto flex my-2">
        <div className="px-2 w-1/2">
          <div className=" mb-4">
            ...
            <input type="state" id="state" maxLength={10} minLength={10} required name="state" className="w-full bg-white rounded border border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out" />
          </div>
        </div>
        <div className="px-2 w-1/2">
          <div className=" mb-4">
            <label htmlFor="pincode" className="leading-7 text-sm text-gray-600">Pincode</label>
            <input type="number" maxLength={8} required id="pincode" name="pincode" className="w-full bg-white rounded border border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out" />
          </div>
        </div>
      </div>
          ...
          {Object.keys(cart).map((k) => {
            return <li key={k}>
              <div className="item flex my-3">
                <div className=" font-semibold  w-1/2 md:w-1/4">{cart[k].name}({cart[k].size}/{cart[k].variant})</div>
                <div className=" flex font-semibold items-center justify-center  w-1/2 md:w-1/4 text-lg "><i className="bi bi-dash-circle-fill cursor-pointer text-blue-500 " onClick={() => { removeFromCart(k, 1, cart[k].price, cart[k].name, cart[k].size, cart[k].variant) }}></i><span className='mx-2 text-sm'>{cart[k].qty}</span><i className="bi cursor-pointer text-blue-500 bi-plus-circle-fill" onClick={() => { addToCart(k, 1, cart[k].price, cart[k].name, cart[k].size, cart[k].variant) }}></i></div>
              </div>
            </li>
          })}
        </ol>
