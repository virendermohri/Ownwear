...
const Shoes = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`${process.env.REACT_APP_PUBLIC_HOST}/api/products/getall?category=Shoes`);
        if (!response.ok) {
        ...
        <section className="text-gray-600 home  mt-[10px] md:mt-[40px]  body-font">
          <div className="container px-5 py-10 bg-white mx-auto">
            <div className="flex flex-wrap -m-4 justify-center">
              {Object.keys(data).length===0 && <p>Sorry all the Shoes are currently out of stock. New stock coming soon.</p>}
