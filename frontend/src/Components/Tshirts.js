...
const Tshirts = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`${process.env.REACT_APP_PUBLIC_HOST}/api/products/getall?category=Tshirts`);
        if (!response.ok) {
      ...
      {!data ? <div className="container flex items-center justify-center mx-auto h-screen ">
        ...
        <section className="text-gray-600 home  mt-[10px] md:mt-[40px]  body-font">
          <div className="container px-5 py-10 bg-white mx-auto">
            ...
            {Object.keys(data).length===0 && <p>Sorry all the Tshirts are currently out of stock. New stock coming soon.</p>}
